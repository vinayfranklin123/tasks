...
class TaskTests(APITestCase):
    ...
    def test_list_tasks_sort_by_creation_time(self):
        Task.objects.create(
            user=self.user,
            name='Another Task',
            description='Another description',
            to_be_completed_time='2024-08-02T10:00:00Z'
        )
        url = reverse('list_tasks') + '?sort_by=created_at'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data[0]['name'], 'Test Task')

    def test_list_tasks_sort_by_completion_time(self):
        completed_task = Task.objects.create(
            user=self.user,
            name='Completed Task',
            description='Completed description',
            to_be_completed_time='2024-07-01T10:00:00Z',
            completed=True,
            completion_time=timezone.now()
        )
        url = reverse('list_tasks') + '?sort_by=completion_time'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
